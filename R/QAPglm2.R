...
#'   \code{family = 'binomial'}, Generalized Methods of Moments is also
#'   available; \code{estimator = 'gmm'}. Currently gmm cannot be combined with
#'   random intercepts.
#
#'
...
QAPglm <- function(formula,
  ...
  if (any(stringr::str_detect(as.character(formula), '\\|'))) {
    if (any(stringr::str_detect(as.character(formula), '\\('))) {
      main  <- all.vars(lme4::reformulas(formula))[-1]
      fe_re <- reformulas::findbars(formula) |>
        lapply(all.vars) |>
        unlist() |>
        unique()
    } else {
      main <- all.vars(formula[[3]][[2]])
      fe_re <- all.vars(formula[[3]][[3]])
    }
  } else {
    main <- predictors
    fe_re <- NULL
  ...
  mod <- paste(deparse(formula),rand_int)
  rand <- any(str_detect(mod, '\\('))
  if ((any(str_detect(mod, '\\|')) && !rand) || !is.null(fixest_se_cluster)) {
    use_fixest <- TRUE
  }

  mod <- formula(paste(mod, collapse = " "))
  fit <- list()
  ...
  if (!rand) {
    ...
    if (use_robust_errors) {
      fit$t <- fit$coefficients / HC3(as.matrix(pred[main]),resid)
    } else {
      if (!use_fixest) {
        fit$t <- summary(base_model)$coefficients[,3]
      } else {
        fit$t <- c(Intercept = NA, fit$coefficients[main] /
                     sqrt(diag(vcov(base_model))))
      }
    }
    names(fit$t) <- c('Intercept',main)
    names(fit$coefficients) <- c('Intercept',main)
  } else {
    if (family == 'gaussian') {
    ...
    resid <- residuals(base_model)
    if (use_robust_errors) {
      fit$t <- fit$coefficients / HC3(pred[main],resid)
    } else {
      fit$t <- summary(base_model)$coefficients[,3]
    }
  }

  if ((nullhyp == "qapspp") && (length(main) == 1)) {
    nullhyp <- "qapy"
  }
