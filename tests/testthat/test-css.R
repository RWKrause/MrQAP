...
test_that("QAP runs with gaussian and qapspp", {
  d <- make_css_test_data()
  fit <- QAP(y ~ x1 + x2, data = d,
...
test_that("QAP works with random intercept for perceiver", {
  d <- make_css_test_data()
  fit <- QAP(y ~ x1, data = d,
                family = "gaussian", nullhyp = "qapy",
                reps = 20, seed = 1,
...
test_that("QAP print method works", {
  d <- make_css_test_data()
  fit <- QAP(y ~ x1, data = d, reps = 20, seed = 1)
  expect_output(print(fit), "CSS")
})

test_that("QAP works with comparison argument", {
  n <- 5
  set.seed(42)
